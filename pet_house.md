...
-   Map to `Chilling.png` or `Sleeping.png` (random choice) animation
-   Update AnimationController state mapping

### 6.2 Implement Drag-to-House Detection

-   Extend `CheckDragToHouse()` method
-   Detect when pet is dropped directly over house button or, if the house panel open, directly over the room
-   Trigger room transition: cat should appear on its bed in a randomly chosen either chilling or sleeping state
...
-   Keep house button visible on desktop (so that room can be hidden or shown)

### 6.4 Pet Animation in Room

-   Render pet sprite inside room view
-   Use `Chilling.png` or `Sleeping.png` animation

### 6.5 Pet Transition out of the room

-   Pet should be grababble, and could be carried out of the room, then it should continue normal behaviour on the desktop

### 6.6 Pet own behaviour

-   Pet should have a random decision to get out of the room, if decision made to get out, it should appear next to the room and should continue normal behaviour on the desktop
-   Room should have a lock (icon under `Resources\elements\control\` locked and unlocked), this element should be positioned directly under the hide button and should have the same size
-   When room unlocked, the lock element should use unlocked.png and cat can make a random decision to get out of the room
-   When room locked, the lock element should use locked.png and cat cannot make a decision to get out of the room
